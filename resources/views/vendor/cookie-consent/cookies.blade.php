                    ...
                    @foreach ($cookies->getCategories() as $category)
                        <div class="cookies__section">
                            ...
                            <a href="#cookies-policy-{{ $category->key() }}"
                                class="cookies__details bg-gradient-to-r from-primary to-secondary hover:text-black text-transparent bg-clip-text">@lang('cookieConsent::cookies.details.more')</a>
                        </div>
                    @endforeach
                </div>
                <div class="cookies__save">
                    <button type="submit"
                        class="rounded-full bg-black py-2.5 px-4 text-white w-full">@lang('cookieConsent::cookies.save')</button>
                </div>
            </form>
        </div>
    </div>
...
<style data-cookie-consent>
    {!! file_get_contents(LCC_ROOT . '/dist/style.css') !!} #cookies-policy .cookies__category input:checked+.cookies__box:after {
        background: linear-gradient(to right, #FF007A, #FF7A00);
    }
